...
def preprocess_input(input_df: pd.DataFrame, reference_df: pd.DataFrame) -> pd.DataFrame:
...
def build_evaluation_matrix(df: pd.DataFrame, model, lr_model) -> tuple[Optional[pd.DataFrame], str]:
    """Build evaluation matrix with classification metrics"""
    needed_columns = ["Gender", "Married", "Dependents", "Education", "Self_Employed", 
...
with st.form("loan_application"):
    ...
    with col2:
        ...
        loan_term = st.selectbox("Loan Amount Term (months)", [360, 480, 300, 240, 180, 120, 60])
...
if submitted:
    ...
    for col in categorical_cols:
        le = LabelEncoder()
        unique_vals = reference_df[col].dropna().unique() if reference_df is not None else []
    ...
    if prediction == 1:
        ...
        st.error(f"❌ **LOAN NOT APPROVED** - Approval Probability: {prediction_proba[1]:.2%}")
...
if reference_df is None:
    ...
    try:
        metrics_df, metrics_note = build_evaluation_matrix(reference_df, rf_model, lr_model)
...
if reference_df is None or "Loan_Approved" not in reference_df.columns:
    ...
    with col1:
        ...
        ax_pie.pie(approval_counts, labels=["Approved", "Not Approved"], autopct="%1.1f%%", colors=["#2ecc71", "#e74c3c"])
    ...
    with col2:
        ...
        plot_df["ApplicantIncome"].hist(bins=30, ax=ax_income, color="skyblue", edgecolor="black")
